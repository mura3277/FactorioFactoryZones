...
function construct_region_color(color_rect)
    ...
    return {
        r = math.floor(color_rect.r * alpha),
        g = math.floor(color_rect.g * alpha),
        b = math.floor(color_rect.b * alpha),
        a = color_rect.a
    }
end
...
script.on_event(
    ...
    function(event)
        ...
        if field and field.valid then field.text = tostring(math.floor(value)) end
...
script.on_event(
    ...
    function(event)
        ...
        if action == "rename_region" then
            ...
            local text = rendering.get_object_by_id(region.text_render_id)
            if text and text.valid then text.text = region.name end
        elseif action == "set_color_channel" then
            ...
            if slider and slider.valid then slider.slider_value = value end
